...
function RecipeCard({ recipe, featured = false, onClick }) {
  ...
  if (featured) {
    return (
      <div
        ...
        role="button"
        tabIndex={0}
        className="text-left border-[1.5px] border-ink rounded bg-paper-deep overflow-hidden flex flex-col md:flex-row cursor-pointer transition-transform hover:-translate-y-1"
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      className="text-left border-[1.5px] border-ink rounded bg-paper-deep p-4 flex items-center gap-4 cursor-pointer transition-transform hover:-translate-y-0.5"
    >
      <img
        src={recipe.strMealThumb}
        alt={recipe.strMeal}
